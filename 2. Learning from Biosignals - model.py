...
def tiny_sleep_net():
    model = nn.Sequential(
        nn.Conv1d(in_channels=1, out_channels=16, kernel_size=64, stride=2, padding=16, bias=False),
        nn.BatchNorm1d(num_features=16, eps=0.001, momentum=0.01),
        nn.ReLU(inplace=True),
        nn.MaxPool1d(kernel_size=4, stride=4),
        nn.Dropout(p=0.25),
        nn.Conv1d(in_channels=16, out_channels=32, kernel_size=8, stride=1, padding=4, bias=False),
        nn.BatchNorm1d(num_features=32, eps=0.001, momentum=0.01),
        nn.ReLU(inplace=True),
        nn.MaxPool1d(kernel_size=2, stride=2),
        nn.Dropout(p=0.25),
        nn.Flatten(),
        nn.Linear(in_features=model[-2].out_features, out_features=5, bias=False)  # Now correctly accessing model's layers
    )
    return model    
...
if __name__ == '__main__':
    model = tiny_sleep_net()

    # Ensure compatibility with TinySleepNet's input shape
    fake_x = torch.randn(size=(2, 1, 3000))  # (batch_size, in_channels, input_length)
